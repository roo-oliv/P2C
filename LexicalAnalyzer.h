...
int main(){
    ...
    Analyzer la;
    la.process();
    Token* tk =  la.getList()->readHeading()->next;
    for(int i = 0; i < la.getList()->getSize(); i++) {
    	cout << tk->lexema << " linha: " << tk->lin << " coluna: " << tk->col << " tipo: " << tk->type << endl;
    	tk = tk->next;
    }
